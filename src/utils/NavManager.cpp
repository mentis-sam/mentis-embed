...
NavScreen colonise_colonising = NavScreen(colonise_colonising_i, colonise_colonising_il, 12, NULL, NULL, &menu_colonise, []() {MachineState::startState(colonisation, DateTime(0, 0, settings.c_timeperiod, 0, 0, 0));}); 
...
NavScreen mycelium_fruiting = NavScreen(mycelium_fruiting_i, mycelium_fruiting_il, 12, NULL, NULL, &menu_colonise, []() {MachineState::startState(colonisation, DateTime(0, 0, settings.f_timeperiod, 0, 0, 0));}); 
...
InputScreen settings_c_time    = InputScreen(settings_timeperiod_i, settings_timeperiod_il, &settings.c_timeperiod, 50);
InputScreen settings_c_temp    = InputScreen(settings_temperature_i, settings_temperature_il, &settings.c_temp, 11);
InputScreen settings_c_airflow = InputScreen(settings_airflow_i, settings_airflow_il, &settings.c_airflow, 10);

InputScreen settings_f_time    = InputScreen(settings_timeperiod_i, settings_timeperiod_il, &settings.f_timeperiod, 50);
InputScreen settings_f_temp    = InputScreen(settings_temperature_i, settings_temperature_il, &settings.f_temp, 11);
InputScreen settings_f_airflow = InputScreen(settings_airflow_i, settings_airflow_il, &settings.f_airflow, 10);
InputScreen settings_f_light   = InputScreen(settings_light_i, settings_light_il, &settings.f_light, 10);
