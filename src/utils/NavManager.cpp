...
NavScreen colonise_colonising = NavScreen(colonise_colonising_i, colonise_colonising_il, 12, NULL, NULL, &menu_colonise, []() {MachineState::startState(colonisation, &Settings::lerpSettings.c_timeperiod);}); 
...
NavScreen mycelium_fruiting = NavScreen(mycelium_fruiting_i, mycelium_fruiting_il, 12, NULL, NULL, &menu_colonise, []() {MachineState::startState(fruiting, &Settings::lerpSettings.f_timeperiod);}); 
...
InputScreen settings_c_time    = InputScreen(settings_timeperiod_i, settings_timeperiod_il, &Settings::settings.c_timeperiod, 50);
InputScreen settings_c_temp    = InputScreen(settings_temperature_i, settings_temperature_il, &Settings::settings.c_temp, 11);
InputScreen settings_c_airflow = InputScreen(settings_airflow_i, settings_airflow_il, &Settings::settings.c_airflow, 10);

InputScreen settings_f_time    = InputScreen(settings_timeperiod_i, settings_timeperiod_il, &Settings::settings.f_timeperiod, 50);
InputScreen settings_f_temp    = InputScreen(settings_temperature_i, settings_temperature_il, &Settings::settings.f_temp, 11);
